...
export async function runEngineV3() {
...
if (process.argv[1] && process.argv[1].endsWith('engine_v3.js')) {
  runEngineV3()
    .then((result) => {
      console.log('[AXIUM:RUNTIME:ENGINE:V3]', JSON.stringify(result, null, 2));
    })
    .catch((error) => {
      console.error('[AXIUM:RUNTIME:ENGINE:V3]', error.message);
      process.exitCode = 1;
    });
}
